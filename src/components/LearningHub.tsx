...
type Subject = 'music' | 'ipa';

const SUBJECTS: { id: Subject; icon: string; label: string; sub: string }[] = [
  { id: 'music', icon: '🎵', label: '音符・コード',   sub: '五線譜の読み方' },
  { id: 'ipa',   icon: '🗣️', label: '発音記号 IPA', sub: '洋楽発音特訓'   },
];
  ...
  return (
    <>
      ...
      <div className="sticky top-[57px] z-20 bg-white/95 backdrop-blur border-b border-gray-100 px-4 py-2">
        <div className="flex gap-1.5 max-w-md mx-auto bg-gray-100 p-1 rounded-2xl">
          {SUBJECTS.map(({ id, icon, label, sub }) => (
            <button key={id} onClick={() => setSubject(id)}
              ...
              <div className="text-left min-w-0">
                <p className="text-[11px] font-black leading-tight truncate">{label}</p>
      ...
      {/* ── コンテンツ ── */}
      <div className="pt-4">
        {subject === 'music' ? <MusicLearning /> : <IPALearning />}
      </div>
    </>
  );
}
